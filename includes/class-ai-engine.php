...
class BR_AI_Engine {
    ...
    public function __construct() {
        $this->api_key = get_option( 'boreal_relay_openai_api_key', '' );
        $this->model   = get_option( 'boreal_relay_model', 'gpt-4o-mini' );
    ...
    private function build_system_prompt() {
        ...
        $support_email = get_option( 'boreal_relay_support_email', get_option( 'admin_email' ) );
        $tone          = get_option( 'boreal_relay_tone', 'friendly and professional' );
        $bot_name      = get_option( 'boreal_relay_bot_name', 'Relay' );

        $this->system_prompt = implode( "\n", array(
            "You are the AI customer service assistant for {$business_name}. Your name is \"{$bot_name}\" — a helpful, knowledgeable assistant who represents the brand.",
            '',
            'PERSONALITY & TONE:',
            "- Be {$tone}",
            '- Keep responses concise (2-4 sentences unless detail is needed)',
            '- Be warm and welcoming while representing the brand with care',
            ...
            '- Treat starter Knowledge Base entries as general guidance only. Do not state site-specific policies, availability, pricing, service areas, or guarantees unless the Knowledge Base explicitly confirms them.',
            '',
            'YOUR ROLE:',
            '- Answer questions about products, services, ordering, pricing, delivery, returns, accounts, and general support',
            ...
            '- The customer explicitly asks to speak with a person or your team',
            '- You truly cannot help at all even with the knowledge base',
            '- It is a legal or billing dispute',
            '',
            'DO NOT escalate for:',
            '- General questions you can partially answer — give the best answer you have and offer to escalate if needed',
            ...
            '- Support: direct customers to the site\'s Contact page or the configured support email',
            '',
            $context,
            '',
            'RESPONSE FORMAT:',
    ...
    public function respond( $message, $history = array(), $page_url = '' ) {
        ...
        foreach ( $greeting_patterns as $pattern ) {
            if ( preg_match( $pattern, trim( $message ) ) ) {
                $bot_name = get_option( 'boreal_relay_bot_name', 'Relay' );
                return array(
                    'reply'      => "Hi there! 👋 I'm {$bot_name}, your Boreal Relay assistant. How can I help you today?",
                    'confidence' => 1.0,
                    'escalate'   => false,
                );
            }
        ...
        if ( empty( $this->api_key ) ) {
            return array(
                'reply'      => 'Our AI assistant is currently being configured. In the meantime, you can reach us through the Contact page on our website and our team will be happy to help!',
                'confidence' => 0,
                'escalate'   => false,
                'api_error'  => true,
            );
    ...
    private function fallback_response() {
        return array(
            'reply'      => 'I\'m having a little trouble connecting right now. Please use the Contact page on our website and our team will help you right away!',
            'confidence' => 0,
            'escalate'   => false,
            'api_error'  => true,
        );
